...
# Store fdr_p_values in file
write.table(format(fdr_df, digits=22), paste(directory, "CCM_data/", file_lang, "_fdr_p_values.csv", sep = ""))

dir.create(paste(directory, "CCM_data/Causal/Graphs", sep = ""), showWarnings = FALSE)

# Perform cross analysis plotting
index <- 1
for(i in 1:(length(cross_child_names))) {
  for(j in 1:(length(cross_mother_names))) {
    # Set variables for cross analysis plotting
...
for(i in 1:(length(cross_child_names))) {
  for(j in 1:(length(cross_child_names))) {
    if(i < j) {
      # Set variables for bootstrap analysis plotting
      name1 <- cross_child_names[[i]]
